...
function BubbleContent({
    ...
    if (collapsible) {
        const isText = getTextFromChildren(props.children);
        const isLong = isText.length > PREVIEW_LENGTH;
        const preview = `${isText.slice(0, PREVIEW_LENGTH)}...`;
        ...
        return (
            ...
            >
                <Collapsible open={open} onOpenChange={setOpen}>
                    <div>{open || !isLong ? isText : preview}</div>
                    {isLong ? (
                        <CollapsibleTrigger
                            render={
                                <Button
                                    variant="link"
                                    className="gap-1 p-0 text-inherit"
                                >
